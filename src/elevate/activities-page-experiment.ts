...
export class ActivitiesPageExperiment extends ElevateDataGenerator implements Experiment {
  ...
  reuseExistingData(): ExperimentSetup {
    ...
    if (!firstPodContext) {
      throw new Error("Cannot reuse activities-page data without an experiment iteration.");
    }
        ...
        for (const arg of iterationConfig.args) {
          ...
          for (const cache of ["no-cache", "indexed-cache"] as const) {
            ...
            const worker = new Worker(__filename, {
              workerData: {logLevel: Logger.getLevel(), podContext: this.podContext, activityLocations, selectedColumns, cache, authorizationMode: this.experimentConfig.authorizationMode}
            });

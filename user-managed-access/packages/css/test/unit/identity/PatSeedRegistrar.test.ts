...
describe('PatSeedRegistrar', (): void => {
  ...
  it('initializes the PAT registrations once the status changes.', async(): Promise<void> => {
    ...
    expect(accountStore.getSetting).toHaveBeenCalledWith('account1', ACCOUNT_SETTINGS_AS_TOKEN);
    expect(accountStore.getSetting).toHaveBeenCalledWith('account1', ACCOUNT_SETTINGS_AUTHZ_SERVER);
    expect(accountStore.getSetting).toHaveBeenCalledWith('account2', ACCOUNT_SETTINGS_AS_TOKEN);
    expect(accountStore.getSetting).toHaveBeenCalledWith('account2', ACCOUNT_SETTINGS_AUTHZ_SERVER);
    expect(umaClient.generateClientCredentials).toHaveBeenCalledTimes(2);
    expect(umaClient.generateClientCredentials).toHaveBeenCalledWith('webId1', 'issuer');
    expect(umaClient.generateClientCredentials).toHaveBeenCalledWith('webId2', 'issuer');
    expect(patUpdater.updateSettings).toHaveBeenCalledTimes(2);
    expect(patUpdater.updateSettings).toHaveBeenCalledWith('account1', 'id', 'secret', 'issuer');
    expect(patUpdater.updateSettings).toHaveBeenCalledWith('account2', 'id', 'secret', 'issuer');
  });
